...
#
package OnlinePetstore::ApiClient;
...
use OnlinePetstore::Configuration;

sub new {
    my $class = shift;

    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'OnlinePetstore::Configuration' ) {
        $config = $_[0];
    } else {
        $config = OnlinePetstore::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("OnlinePetstore::Object::$class")->new;
        if (ref $data eq "HASH") {
